...
-- 6) Clientes: unicidad correcta por workspace, sin bloquear mismo nombre entre scopes.
...
create unique index if not exists clients_scope_name_unique
  on public.clients (
    ...
    coalesce(account_owner_id::text, 'none'),
    lower(trim(name))
  );

-- 7) RLS workspace-aware para tareas/proyectos dentro de organizaciones.
...
create policy projects_select_workspace_access on public.projects
...
create policy projects_update_workspace_editor on public.projects
for update to authenticated using (
  owner_id = auth.uid()
  or has_project_role(id, auth.uid(), array['owner','editor'])
...
create policy projects_delete_workspace_owner on public.projects
...
create policy tasks_select_workspace_access on public.tasks
for select to authenticated using (
  owner_id = auth.uid()
...
create policy tasks_update_workspace_access on public.tasks
for update to authenticated using (
  owner_id = auth.uid()
  or is_task_assignee(id, auth.uid())
  or ((project_id is not null) and is_project_member(project_id, auth.uid()))
...
create policy tasks_delete_workspace_owner on public.tasks
for delete to authenticated using (
  owner_id = auth.uid()
  or (organization_id is not null and public.is_org_admin(organization_id))
);
