...
if (pkg.scripts?.["verify:current"] !== "npm run verify:v58.28.18.1") failures.push("verify:current must target verify:v58.28.18.1");
...
if (!versionText.includes(expectedVersion)) failures.push("version.ts must contain v58.28.18.1 slug");
...
if (failures.length) {
  console.error("[verify:v58.28.18.1] FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("[verify:v58.28.18.1] OK — dependency security public registry lockfile hotfix aligned.");
