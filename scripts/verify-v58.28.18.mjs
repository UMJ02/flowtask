...
if (pkg.scripts?.["verify:current"] !== "npm run verify:v58.28.19", "npm run verify:v58.28.20", "npm run verify:v58.28.21", "npm run verify:v58.28.21") failures.push("verify:current must target verify:v58.28.18");
...
if (!versionText.includes(expectedVersion)) failures.push("version.ts must contain v58.28.18 slug");
if (!lockText.includes('"next": "15.5.18"')) failures.push("package-lock must contain Next 15.5.18");
if (!lockText.includes('"postcss": "8.5.14"')) failures.push("package-lock must contain PostCSS 8.5.14 override");
...
if (failures.length) {
  console.error("[verify:v58.28.18] FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("[verify:v58.28.18] OK — dependency security audit and safe Next patch aligned.");
