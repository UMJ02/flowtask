...
if (missing.length) {
  console.error("[verify:v58.15.4] Missing files:", missing.join(", "));
  process.exit(1);
}
...
const checks = [
  ["release version aligned", release.includes("58.15.4.1-media-system-typefix")],
...
if (failed.length) {
  console.error("[verify:v58.15.4] Failed checks:");
  for (const [label] of failed) console.error(`- ${label}`);
  process.exit(1);
}
console.log("[verify:v58.15.4] OK — Media system final aligned.");
