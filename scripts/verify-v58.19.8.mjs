...
mustInclude('package.json', '"version": "58.19.8-final-user-readiness"', 'v58.19.8 package version');
mustInclude('package.json', '"verify:current": "npm run verify:v58.19.8"', 'current verifier target');
mustInclude('package.json', '"verify:v58.19.8": "node scripts/verify-v58.19.8.mjs"', 'v58.19.8 verifier script');
mustInclude('package-lock.json', '"version": "58.19.8-final-user-readiness"', 'lockfile version');
mustInclude('src/lib/release/version.ts', '58.19.8-final-user-readiness', 'runtime version');
mustInclude('src/lib/release/version.ts', 'v58.19.8 Final User Readiness', 'runtime release name');
mustInclude('src/lib/release/version.ts', 'production-candidate', 'runtime release stage');
mustInclude('README.md', 'v58.19.8 Final User Readiness', 'README release title');
...
mustInclude('scripts/build-deploy-readiness.mjs', '58.19.8-final-user-readiness', 'deploy readiness current version');
mustInclude('scripts/build-deploy-readiness.mjs', 'verify:v58.19.8', 'deploy readiness current verifier');
...
mustInclude('docs/releases/RELEASE_NOTES_v58.19.8.md', 'Final User Readiness', 'release notes');
mustInclude('docs/qa/FLOWTASK_V58.19.8_FINAL_USER_READINESS_SMOKE.md', 'Aislamiento personal ↔ organización', 'manual workspace smoke checklist');
mustInclude('docs/qa/FLOWTASK_V58.19.8_FINAL_USER_READINESS_SMOKE.md', 'doctor:supabase', 'Supabase live smoke command');
...
if (failures.length) {
  console.error('[verify:v58.19.8] FAIL');
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log('[verify:v58.19.8] OK — Final User Readiness aligned: metadata, release checks, XLSX, landing, personal/org isolation and Supabase smoke docs.');
