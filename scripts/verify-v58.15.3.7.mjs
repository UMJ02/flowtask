...
if (missing.length) {
  console.error("[verify:v58.15.3.7] Missing files:", missing.join(", "));
  process.exit(1);
}
...
const checks = [
  ["release version aligned", release.includes("58.15.3.7-task-editor-edge-alignment")],
  ["task editor uses edge-aligned canvas", taskForm.includes("flex h-[64px] w-full flex-col") && taskForm.includes("px-4 sm:px-5 lg:px-6") && !taskForm.includes("max-w-[1840px]")],
...
if (failed.length) {
  console.error("[verify:v58.15.3.7] Failed checks:");
  for (const [label] of failed) console.error(`- ${label}`);
  process.exit(1);
}
console.log("[verify:v58.15.3.7] OK — Task editor premium edge alignment + project/client media aligned.");
