...
if (missing.length) {
  console.error("[verify:v58.15.3.5] Missing files:", missing.join(", "));
  process.exit(1);
}
...
const checks = [
  ["release version aligned", release.includes("58.15.3.5-task-editor-project-client-media")],
...
if (failed.length) {
  console.error("[verify:v58.15.3.5] Failed checks:");
  for (const [label] of failed) console.error(`- ${label}`);
  process.exit(1);
}
console.log("[verify:v58.15.3.5] OK — Task editor premium + project/client media aligned.");
