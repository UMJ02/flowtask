...
if (pkg.scripts?.["verify:current"] !== "npm run verify:v58.28.21.1") failures.push("verify:current must target verify:v58.28.21.1");
...
if (failures.length) {
  console.error("[verify:v58.28.21.1] FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log("[verify:v58.28.21.1] OK — Classic project detail view/edit UX polish aligned.");
