...
const failures = [];
const expectedVersion = "58.24.8-boards-hero-diagram-visual-refresh";
...
scripts["verify:current"] === "npm run verify:v58.24.8" ? pass("verify current aligned") : fail("verify:current must target verify:v58.24.8");
scripts["verify:v58.24.8"] === "node scripts/verify-v58.24.8.mjs" ? pass("version verifier available") : fail("verify:v58.24.8 script missing or incorrect");
exists("scripts/verify-v58.24.8.mjs") ? pass("verify-v58.24.8 script exists") : fail("scripts/verify-v58.24.8.mjs missing");
read("src/lib/release/version.ts").includes(expectedVersion) ? pass("runtime version export aligned") : fail("src/lib/release/version.ts must export v58.24.8");
exists("docs/release/V58_24_8_BOARDS_HERO_DIAGRAM_VISUAL_REFRESH.md") ? pass("release notes available") : fail("release notes missing");
exists("docs/qa/FLOWTASK_V58_24_8_BOARDS_HERO_DIAGRAM_VISUAL_REFRESH_QA.md") ? pass("QA document available") : fail("QA doc missing");
...
read("package-lock.json").includes(expectedVersion) ? pass("package-lock version aligned") : fail("package-lock.json must include v58.24.8 package version");
...
console.log("[deploy-production-readiness] OK — v58.24.8 production readiness aligned.");
