...
const failures = [];
const expectedVersion = "58.24.2-board-premium-toolbar-clear-canvas-actions";
...
scripts["verify:current"] === "npm run verify:v58.24.2" ? pass("verify current aligned") : fail("verify:current must target verify:v58.24.2");
scripts["verify:v58.24.2"] === "node scripts/verify-v58.24.2.mjs" ? pass("version verifier available") : fail("verify:v58.24.2 script missing or incorrect");
exists("scripts/verify-v58.24.2.mjs") ? pass("verify-v58.24.2 script exists") : fail("scripts/verify-v58.24.2.mjs missing");
read("src/lib/release/version.ts").includes(expectedVersion) ? pass("runtime version export aligned") : fail("src/lib/release/version.ts must export v58.24.2");
exists("docs/release/V58_24_2_BOARD_PREMIUM_TOOLBAR_CLEAR_CANVAS_ACTIONS.md") ? pass("release notes available") : fail("release notes missing");
exists("docs/qa/FLOWTASK_V58_24_2_BOARD_PREMIUM_TOOLBAR_QA.md") ? pass("QA document available") : fail("QA doc missing");
exists("docs/boards/FLOWTASK_BOARD_PREMIUM_TOOLBAR_CLEAR_ACTIONS.md") ? pass("premium toolbar doc available") : fail("premium toolbar doc missing");
...
read("package-lock.json").includes(expectedVersion) ? pass("package-lock version aligned") : fail("package-lock.json must include v58.24.2 package version");
...
console.log("[deploy-production-readiness] OK — v58.24.2 production readiness aligned.");
