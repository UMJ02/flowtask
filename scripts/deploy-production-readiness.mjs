...
const expectedVerifyCurrent = "npm run verify:v58.21.7";
...
scripts["verify:current"] === expectedVerifyCurrent ? pass("verify current aligned") : fail("verify:current must target verify:v58.21.7");
scripts["verify:v58.21.7"] === "node scripts/verify-v58.21.7.mjs" ? pass("version verifier available") : fail("verify:v58.21.7 script missing or incorrect");
fileExists("scripts/verify-v58.21.7.mjs") ? pass("verify-v58.21.7 script exists") : fail("scripts/verify-v58.21.7.mjs missing");
fileIncludes("src/lib/release/version.ts", expectedVersion) ? pass("runtime version export aligned") : fail("src/lib/release/version.ts must export v58.21.7");
...
fileExists("docs/release/V58_21_7_STYLE_CASCADE_RESET_INTERACTION_POLISH.md") ? pass("release notes available") : fail("v58.21.7 release notes missing");
fileExists("docs/qa/FLOWTASK_V58_21_7_STYLE_CASCADE_QA.md") ? pass("QA document available") : fail("v58.21.7 QA doc missing");
fileExists("docs/design-system/FLOWTASK_STYLE_CASCADE.md") ? pass("style cascade design system document available") : fail("style cascade design system document missing");
fileIncludes("src/lib/design-system/tokens.ts", "cascade-reset-compact") ? pass("cascade reset density token available") : fail("cascade reset density token missing");
fileIncludes("src/lib/design-system/tokens.ts", "cascadePolicy") ? pass("cascade policy available") : fail("cascade policy missing");
...
fileIncludes("src/app/globals.css", ".ft-motion-expand") ? pass("expand motion utility available") : fail("expand motion utility missing");
...
fileIncludes("package-lock.json", expectedVersion) ? pass("package-lock version aligned") : fail("package-lock.json must include v58.21.7 package version");
const migrations = fileExists("supabase/migrations") ? fs.readdirSync(path.join(root, "supabase/migrations")) : [];
if (migrations.some((file) => file.includes("58_21_7") || file.includes("v58_21_7"))) fail("v58.21.7 must not add Supabase migrations"); else pass("no Supabase migration added");
...
console.log("[deploy-production-readiness] OK — v58.21.7 production readiness aligned.");
