...
scripts["verify:current"] === "npm run verify:v58.24.1" ? pass("verify current aligned") : fail("verify:current must target verify:v58.24.1");
scripts["verify:v58.24.1"] === "node scripts/verify-v58.24.1.mjs" ? pass("version verifier available") : fail("verify:v58.24.1 script missing or incorrect");
exists("scripts/verify-v58.24.1.mjs") ? pass("verify-v58.24.1 script exists") : fail("scripts/verify-v58.24.1.mjs missing");
read("src/lib/release/version.ts").includes(expectedVersion) ? pass("runtime version export aligned") : fail("src/lib/release/version.ts must export v58.24.1");
exists("docs/release/V58_24_1_BOARD_EDITOR_UX_ELEMENT_CONTROLS_HARDENING.md") ? pass("release notes available") : fail("release notes missing");
exists("docs/qa/FLOWTASK_V58_24_1_BOARD_EDITOR_UX_QA.md") ? pass("QA document available") : fail("QA doc missing");
exists("docs/boards/FLOWTASK_BOARD_EDITOR_UX_HARDENING.md") ? pass("board editor UX doc available") : fail("board editor UX doc missing");
...
read("package-lock.json").includes(expectedVersion) ? pass("package-lock version aligned") : fail("package-lock.json must include v58.24.1 package version");
...
console.log("[deploy-production-readiness] OK — v58.24.1 production readiness aligned.");
