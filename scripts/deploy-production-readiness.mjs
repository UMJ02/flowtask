...
scripts["verify:current"] === "npm run verify:v58.23.2" ? pass("verify current aligned") : fail("verify:current must target verify:v58.23.2");
scripts["verify:v58.23.2"] === "node scripts/verify-v58.23.2.mjs" ? pass("version verifier available") : fail("verify:v58.23.2 script missing or incorrect");
exists("scripts/verify-v58.23.2.mjs") ? pass("verify-v58.23.2 script exists") : fail("scripts/verify-v58.23.2.mjs missing");
read("src/lib/release/version.ts").includes(expectedVersion) ? pass("runtime version export aligned") : fail("src/lib/release/version.ts must export v58.23.2");
exists("docs/release/V58_23_2_BOARD_TABLES_INLINE_EDITING.md") ? pass("release notes available") : fail("release notes missing");
exists("docs/qa/FLOWTASK_V58_23_2_BOARD_TABLES_QA.md") ? pass("QA document available") : fail("QA doc missing");
...
read("package-lock.json").includes(expectedVersion) ? pass("package-lock version aligned") : fail("package-lock.json must include v58.23.2 package version");
if (failures.length) {
  console.error("[deploy-production-readiness] Failed checks:");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}
console.log("[deploy-production-readiness] OK — v58.23.2 production readiness aligned.");
