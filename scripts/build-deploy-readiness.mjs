...
const expectedReleaseLabel = "v58.24.8 Boards Hero Diagram Visual Refresh";
...
for (const rel of [
  ...
  "scripts/runtime-check.mjs","scripts/validate-env.mjs","scripts/verify-v58.24.8.mjs",
...
for (const scriptName of ["build","vercel:build","deploy:readiness","build:preflight","verify:current","deploy:production:ready","verify:v58.24.8"]) if (!scripts[scriptName]) failures.push(`Missing package script: ${scriptName}`);
if (pkg.version !== expectedVersion) failures.push(`Unexpected package version: ${pkg.version}`);
if (scripts["verify:current"] !== "npm run verify:v58.24.8") failures.push("verify:current must target verify:v58.24.8");
if (scripts["verify:v58.24.8"] !== "node scripts/verify-v58.24.8.mjs") failures.push("verify:v58.24.8 must target scripts/verify-v58.24.8.mjs");
...
console.log("[build-deploy-readiness] OK — v58.24.8 package, env and boards home action readiness aligned.");
