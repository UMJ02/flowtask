...
requireFile("scripts/verify-v58.20.mjs");
...
for (const scriptName of ["build", "vercel:build", "deploy:readiness", "build:preflight", "verify:current", "deploy:production:ready", "verify:v58.20"]) {
  if (!scripts[scriptName]) failures.push(`Missing package script: ${scriptName}`);
}
if (pkg.version !== expectedVersion) failures.push(`Unexpected package version: ${pkg.version}`);
if (scripts["verify:current"] !== "npm run verify:v58.20") failures.push("verify:current must target verify:v58.20");
if (scripts["verify:v58.20"] !== "node scripts/verify-v58.20.mjs") failures.push("verify:v58.20 must target scripts/verify-v58.20.mjs");
