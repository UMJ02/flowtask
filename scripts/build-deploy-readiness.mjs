...
const expectedReleaseLabel = "v58.24.2 Board Premium Toolbar + Clear Canvas Actions";
...
for (const rel of [
  ...
  "scripts/runtime-check.mjs","scripts/validate-env.mjs","scripts/verify-v58.24.2.mjs",
  "docs/release/V58_24_2_BOARD_PREMIUM_TOOLBAR_CLEAR_CANVAS_ACTIONS.md",
  "docs/qa/FLOWTASK_V58_24_2_BOARD_PREMIUM_TOOLBAR_QA.md",
  "docs/boards/FLOWTASK_BOARD_PREMIUM_TOOLBAR_CLEAR_ACTIONS.md",
...
for (const scriptName of ["build","vercel:build","deploy:readiness","build:preflight","verify:current","deploy:production:ready","verify:v58.24.2"]) if (!scripts[scriptName]) failures.push(`Missing package script: ${scriptName}`);
if (pkg.version !== expectedVersion) failures.push(`Unexpected package version: ${pkg.version}`);
if (scripts["verify:current"] !== "npm run verify:v58.24.2") failures.push("verify:current must target verify:v58.24.2");
if (scripts["verify:v58.24.2"] !== "node scripts/verify-v58.24.2.mjs") failures.push("verify:v58.24.2 must target scripts/verify-v58.24.2.mjs");
...
console.log("[build-deploy-readiness] OK — v58.24.2 package, env and premium toolbar readiness aligned.");
