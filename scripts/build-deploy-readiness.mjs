...
const expectedReleaseLabel = "v58.24.4 Board Premium Visual System + Motion Polish";
...
for (const rel of [
  ...
  "scripts/runtime-check.mjs","scripts/validate-env.mjs","scripts/verify-v58.24.4.mjs",
  "docs/release/V58_24_4_BOARD_PREMIUM_VISUAL_SYSTEM_MOTION_POLISH.md",
  "docs/qa/FLOWTASK_V58_24_4_BOARD_PREMIUM_VISUAL_QA.md",
...
for (const scriptName of ["build","vercel:build","deploy:readiness","build:preflight","verify:current","deploy:production:ready","verify:v58.24.4"]) if (!scripts[scriptName]) failures.push(`Missing package script: ${scriptName}`);
if (pkg.version !== expectedVersion) failures.push(`Unexpected package version: ${pkg.version}`);
if (scripts["verify:current"] !== "npm run verify:v58.24.4") failures.push("verify:current must target verify:v58.24.4");
if (scripts["verify:v58.24.4"] !== "node scripts/verify-v58.24.4.mjs") failures.push("verify:v58.24.4 must target scripts/verify-v58.24.4.mjs");
...
console.log("[build-deploy-readiness] OK — v58.24.4 package, env and premium visual system readiness aligned.");
