...
const expectedReleaseLabel = "v58.24.6 Boards Home UI Redesign + Saved Board Delete";
...
for (const rel of [
  ...
  "scripts/runtime-check.mjs","scripts/validate-env.mjs","scripts/verify-v58.24.6.mjs",
  "docs/release/V58_24_6_BOARDS_HOME_UI_REDESIGN_SAVED_BOARD_DELETE.md",
  "docs/qa/FLOWTASK_V58_24_6_BOARDS_HOME_QA.md",
...
for (const scriptName of ["build","vercel:build","deploy:readiness","build:preflight","verify:current","deploy:production:ready","verify:v58.24.6"]) if (!scripts[scriptName]) failures.push(`Missing package script: ${scriptName}`);
if (pkg.version !== expectedVersion) failures.push(`Unexpected package version: ${pkg.version}`);
if (scripts["verify:current"] !== "npm run verify:v58.24.6") failures.push("verify:current must target verify:v58.24.6");
if (scripts["verify:v58.24.6"] !== "node scripts/verify-v58.24.6.mjs") failures.push("verify:v58.24.6 must target scripts/verify-v58.24.6.mjs");
...
console.log("[build-deploy-readiness] OK — v58.24.6 package, env and boards home readiness aligned.");
