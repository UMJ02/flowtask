...
const expectedReleaseLabel = "v58.24.8.3 Boards Home Style Polish";
...
for (const rel of [
  ...
  "scripts/runtime-check.mjs","scripts/validate-env.mjs","scripts/verify-v58.24.8.3.mjs",
...
for (const scriptName of ["build","vercel:build","deploy:readiness","build:preflight","verify:current","deploy:production:ready","verify:v58.24.8.3"]) {
  if (!scripts[scriptName]) failures.push(`Missing package script: ${scriptName}`);
}
if (pkg.version !== expectedVersion) failures.push(`Unexpected package version: ${pkg.version}`);
if (scripts["verify:current"] !== "npm run verify:v58.24.8.3") failures.push("verify:current must target verify:v58.24.8.3");
if (scripts["verify:v58.24.8.3"] !== "node scripts/verify-v58.24.8.3.mjs") failures.push("verify:v58.24.8.3 must target scripts/verify-v58.24.8.3.mjs");
...
console.log("[build-deploy-readiness] OK — v58.24.8.3 package, env and boards home style polish readiness aligned.");
