...
const expectedReleaseLabel = "v58.23.6 Board Anchored Comments + Files";
...
for (const rel of [
  ...
  "scripts/runtime-check.mjs","scripts/validate-env.mjs","scripts/verify-v58.23.6.mjs",
...
for (const scriptName of ["build","vercel:build","deploy:readiness","build:preflight","verify:current","deploy:production:ready","verify:v58.23.6"]) if (!scripts[scriptName]) failures.push(`Missing package script: ${scriptName}`);
if (pkg.version !== expectedVersion) failures.push(`Unexpected package version: ${pkg.version}`);
if (scripts["verify:current"] !== "npm run verify:v58.23.6") failures.push("verify:current must target verify:v58.23.6");
if (scripts["verify:v58.23.6"] !== "node scripts/verify-v58.23.6.mjs") failures.push("verify:v58.23.6 must target scripts/verify-v58.23.6.mjs");
...
console.log("[build-deploy-readiness] OK — v58.23.6 package, env and board anchored comments/files readiness aligned.");
