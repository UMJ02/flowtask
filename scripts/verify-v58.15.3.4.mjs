...
if (missing.length) {
  console.error("[verify:v58.15.3.4] Missing files:", missing.join(", "));
  process.exit(1);
}
...
const checks = [
  ["release version aligned", release.includes("58.15.3.4-select-scope-stability-fix")],
...
if (failed.length) {
  console.error("[verify:v58.15.3.4] Failed checks:");
  for (const [label] of failed) console.error(`- ${label}`);
  if (forbiddenFound.length) console.error(`- Forbidden fields found: ${forbiddenFound.join(", ")}`);
  process.exit(1);
}

console.log("[verify:v58.15.3.4] OK — Select Scope Stability Fix aligned with DB-safe fields.");
