...
if (missing.length) {
  console.error("[verify:v58.15.3.9] Missing files:", missing.join(", "));
  process.exit(1);
}
...
const checks = [
  ["release version aligned", release.includes("58.15.3.9-project-list-title-input-fix")],
...
if (failed.length) {
  console.error("[verify:v58.15.3.9] Failed checks:");
  for (const [label] of failed) console.error(`- ${label}`);
  process.exit(1);
}
console.log("[verify:v58.15.3.9] OK — Project listing access and task title input spacing aligned.");
