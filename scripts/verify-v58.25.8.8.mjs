...
const expectedRelease = "v58.25.8.8 Workspace Project Activity + Files Context Polish";
const pkg = JSON.parse(read("package.json"));

if (pkg.version !== expectedVersion) failures.push("package version must be v58.25.8.8 workspace project activity files context polish");
if ((pkg.scripts ?? {})["verify:current"] !== "npm run verify:v58.25.8.8") failures.push("verify:current must target verify:v58.25.8.8");
if ((pkg.scripts ?? {})["verify:v58.25.8.8"] !== "node scripts/verify-v58.25.8.8.mjs") failures.push("verify:v58.25.8.8 script missing");
...
requireFile("docs/release/V58_25_8_8_WORKSPACE_PROJECT_ACTIVITY_FILES_CONTEXT_POLISH.md");
requireFile("docs/qa/FLOWTASK_V58_25_8_8_WORKSPACE_PROJECT_ACTIVITY_FILES_CONTEXT_POLISH_QA.md");

if (failures.length) {
  console.error("[verify:v58.25.8.8] FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("[verify:v58.25.8.8] OK — Workspace project activity + files context polish aligned without new DB tables.");
