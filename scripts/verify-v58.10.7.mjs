...
for (const file of requiredFiles) {
  if (!fs.existsSync(path.join(root, file))) {
    console.error(`[verify-v58.10.7] Missing ${file}`);
    failures += 1;
  }
}

if (failures > 0) {
  console.error("[verify-v58.10.7] Repo verification failed.");
  process.exit(1);
}

console.log("[verify-v58.10.7] OK — workspace switch personal + organización listo para continuidad moderna.");
