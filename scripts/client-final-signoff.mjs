...
if (missing.length) {
  console.error("[release:final] Missing artifacts:\n- " + missing.join("\n- "));
  process.exit(1);
}

const signoff = fs.readFileSync(path.join(root, "V_Report/CLIENT_FINAL_SIGNOFF_v54.0.md"), "utf8");
const sections = [
  "Seguridad y acceso",
  "QA funcional",
  "UX/UI cliente",
...
if (missingSections.length) {
  console.error("[release:final] Signoff doc incomplete:\n- " + missingSections.join("\n- "));
  process.exit(1);
}

console.log("[release:final] OK · Client Final signoff bundle present.");
