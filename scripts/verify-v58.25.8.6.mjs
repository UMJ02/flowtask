...
const expectedVersion = "58.25.8.6-workspace-responsive-polish-right-panel-ux";
const expectedRelease = "v58.25.8.6 Workspace Responsive Polish + Right Panel UX";
const pkg = JSON.parse(read("package.json"));

if (pkg.version !== expectedVersion) failures.push("package version must be v58.25.8.6 workspace responsive polish + right panel ux");
if ((pkg.scripts ?? {})["verify:current"] !== "npm run verify:v58.25.8.6") failures.push("verify:current must target verify:v58.25.8.6");
if ((pkg.scripts ?? {})["verify:v58.25.8.6"] !== "node scripts/verify-v58.25.8.6.mjs") failures.push("verify:v58.25.8.6 script missing");
...
requireIncludes("src/app/globals.css", "v58.25.8.6 Workspace Responsive Polish + Right Panel UX");
...
requireFile("docs/release/V58_25_8_6_WORKSPACE_RESPONSIVE_POLISH_RIGHT_PANEL_UX.md");
requireFile("docs/qa/FLOWTASK_V58_25_8_6_WORKSPACE_RESPONSIVE_POLISH_RIGHT_PANEL_UX_QA.md");

if (failures.length) {
  console.error("[verify:v58.25.8.6] FAIL");
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log("[verify:v58.25.8.6] OK — Workspace responsive polish and right panel UX aligned without new DB tables.");
