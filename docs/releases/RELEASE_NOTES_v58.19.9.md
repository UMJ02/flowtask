...
- Metadata de release actualizada a `58.19.9-supabase-live-qa-vercel-verify-fix`.
- `verify:current` ahora apunta a `verify:v58.19.9`.
...
- `verify:current` ahora es seguro para Vercel/local: no falla por `node_modules`, `.env`, `.env.local` o `.next` durante builds reales.
- La validación de paquete ZIP limpio queda disponible con `FLOWTASK_VERIFY_RELEASE_PACKAGE=1 npm run verify:v58.19.9`.
- `vercel:build` puede ejecutar `build:preflight` sin bloquear por carpetas generadas durante instalación.
