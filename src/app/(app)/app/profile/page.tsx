  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-2 text-sm text-slate-300 md:text-base">
              Gestiona nombre, correo, contraseña y foto de perfil desde una vista más clara, estable y cómoda en desktop y móvil.
            </p>
          </div>

          <div className="grid gap-3 md:grid-cols-2 xl:min-w-[560px] xl:max-w-[760px] xl:flex-1">
            {heroItems.map((item) => {
              const Icon = item.icon;
              return (
                <div key={item.label} className="min-w-0 rounded-[26px] bg-white/10 px-5 py-4 ring-1 ring-white/10">
                  <div className="flex items-center gap-2 text-emerald-300">
                    <Icon className="h-4 w-4" />
                    <p className="text-xs uppercase tracking-[0.16em]">{item.label}</p>
                  </div>
                  <p className="mt-3 break-words text-xl font-semibold leading-tight text-white md:text-2xl">{item.value}</p>
                </div>
              );
            })}
          </div>
      ...
      <Card>
        ...
        <p className="mt-1 text-sm text-slate-500">Aquí solo ves datos personales y seguridad básica. La configuración operativa sigue separada en Settings.</p>
        <div className="mt-4">
          <ProfileSettingsForm
