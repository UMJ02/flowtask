...
type Tone = 'critical' | 'attention' | 'stable';
...
export type ExecutiveSuiteSummary = {
  ...
  decisionBoard: Array<{
    title: string;
    detail: string;
    source: 'Risk Radar' | 'Intelligence' | 'Workspace OS' | 'Execution Center';
    tone: Tone;
  }>;
...
export async function getExecutiveSuiteSummary(): Promise<ExecutiveSuiteSummary | null> {
  ...
  const decisionBoard: ExecutiveSuiteSummary['decisionBoard'] = [
    ...
    ...risk.recommendations.slice(0, 1).map((item) => ({
      ...
      tone: (risk.kpis.riskScore >= 65 ? 'critical' : 'attention') as Tone,
    })),
  ...
  const governanceWatchlist: ExecutiveSuiteSummary['governanceWatchlist'] = [
    ...reports.projectWatchlist.slice(0, 3).map((item) => ({
      ...
      tone: (item.urgency === 'overdue' ? 'critical' : item.urgency === 'this_week' ? 'attention' : 'stable') as Tone,
    })),
    ...
    ...reports.focusTasks.slice(0, 2).map((item) => ({
      ...
      tone: (item.urgency === 'overdue' ? 'critical' : item.urgency === 'today' ? 'attention' : 'stable') as Tone,
    })),
  ].slice(0, 7);
  ...
  const weeklyCadence: ExecutiveSuiteSummary['weeklyCadence'] = [
    ...
    },
    {
      ...
      detail: `${reports.kpis.completionRate}% de cierre con ${reports.kpis.activeTasks} tarea(s) activas y ${reports.kpis.activeProjects} proyecto(s) activos.`,
    },
  ];
