...
import { BellOff, ChevronDown, ChevronUp, ShieldCheck, Sparkles, X } from "lucide-react";
...
export function IntelligentAttentionAssistant({ workspaceKey, workspaceLabel = "tu workspace", location = "dashboard", signals, notificationsEnabled = true }: Props) {
  ...
  return (
    <Card
      className={[
        "relative overflow-hidden rounded-[30px] border p-0 shadow-[0_20px_48px_rgba(15,23,42,0.08)]",
    ...
    >
      <div className="grid gap-0 lg:grid-cols-[220px_minmax(0,1fr)]">
        {settings.showMascot ? (
          <div className="relative hidden min-h-[220px] overflow-hidden bg-[radial-gradient(circle_at_top,_rgba(255,255,255,0.94),rgba(248,250,252,0.72))] lg:block">
        ...
        <div className="relative px-4 py-4 sm:px-5 sm:py-5 lg:px-6 lg:py-6">
          ...
          <div className="mt-4 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
            <div className="max-w-3xl">
              <div className="flex items-start gap-3">
                <span className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl border border-white/80 bg-white/90 text-slate-900 shadow-sm">
                  <Icon className="h-5 w-5" />
                </span>
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="rounded-full bg-slate-900 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.16em] text-white">{activeCard.toneLabel}</span>
                  ...
                  <h2 className="mt-2 text-[1.45rem] font-semibold tracking-tight text-slate-950">{activeCard.title}</h2>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                ...
                <Button type="button" variant="secondary" className="rounded-full" onClick={() => snooze()}>
                  Ver luego
                </Button>
            ...
            <div className="grid gap-2.5 rounded-[24px] border border-white/75 bg-white/78 p-3.5 text-sm text-slate-600 shadow-sm xl:w-[280px]">
              <div className="flex items-center gap-2 text-slate-900">
                <ShieldCheck className="h-4 w-4 text-emerald-600" />
                <p className="font-semibold">Tu configuración actual</p>
              </div>
              <div className="grid gap-2">
                <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2 ring-1 ring-slate-200/70">
                  <span>Modo</span>
                  <span className="font-semibold text-slate-900">{settings.sensitivity}</span>
                </div>
                <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2 ring-1 ring-slate-200/70">
                  <span>Descanso</span>
                  <span className="font-semibold text-slate-900">{settings.cadenceMinutes} min</span>
                </div>
                <div className="flex items-center justify-between rounded-2xl bg-slate-50 px-3 py-2 ring-1 ring-slate-200/70">
                  <span>Workspace</span>
                  <span className="font-semibold text-slate-900">{settings.allowWorkspaceSpecificHistory ? "Aislado" : "Global"}</span>
                </div>
              </div>
              <div className="inline-flex items-center gap-2 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 ring-1 ring-slate-200/70">
          ...
          {expanded ? (
            <div className="mt-5 rounded-[24px] border border-white/75 bg-white/78 p-3 sm:p-4">
