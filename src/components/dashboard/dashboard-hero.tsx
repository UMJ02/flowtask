...
export function DashboardHero({
  ...
  const focusLabel = overdueTasks > 0 ? 'Tu foco inmediato está en resolver vencimientos.' : dueSoonTasks > 0 ? 'Tienes entregas cercanas que conviene anticipar hoy.' : 'Tu operación está estable; puedes avanzar en trabajo profundo.';
  ...
  return (
    <Card className="rounded-[30px] border border-slate-200/80 bg-[radial-gradient(circle_at_top_left,_rgba(16,185,129,0.14),_transparent_28%),linear-gradient(135deg,rgba(255,255,255,0.98),rgba(248,250,252,0.98))] p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)] md:p-7">
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1.3fr)_minmax(360px,0.9fr)] xl:items-start">
        <div>
          <div className="inline-flex items-center gap-2 rounded-full bg-white/85 px-3 py-1 text-xs font-semibold tracking-[0.16em] text-emerald-700 ring-1 ring-emerald-100">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {metrics.map((metric) => {
            ...
            return (
              <div key={metric.label} className="rounded-[24px] border border-white/70 bg-white/90 p-4 shadow-[0_10px_24px_rgba(15,23,42,0.04)] ring-1 ring-slate-100 backdrop-blur">
