...
function InteractiveDashboardBoardComponent() {
  ...
  async function updateTaskStatus(taskId: string, nextStatus: 'en_proceso' | 'en_espera' | 'concluido') {
    ...
    const currentTask = boardTasks.find((item) => item.id === taskId) ?? null;
    ...
    setDataError(null);
    setBoardTasks((current) => sortTasksByDueDate(current.map((item) => (item.id === taskId ? { ...item, status: nextStatus, due_date: nextDueDate } : item))));

    const { error } = await supabase.from('tasks').update(buildTaskStatusUpdate(nextStatus, currentTask?.due_date)).eq('id', taskId);
