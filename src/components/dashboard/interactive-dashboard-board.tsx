...
function InteractiveDashboardBoardComponent() {
  ...
  async function updateTaskStatus(taskId: string, nextStatus: 'en_proceso' | 'en_espera' | 'concluido') {
    ...
    const targetTask = previousTasks.find((item) => item.id === taskId) ?? null;
    const nextDate = nextStatus === 'en_espera' ? (targetTask?.due_date ?? null) : isoDate(new Date());
    setStatusUpdatingTaskId(taskId);
    setDataError(null);
    setBoardTasks((current) => sortTasksByDueDate(current.map((item) => (item.id === taskId ? { ...item, status: nextStatus, due_date: nextDate } : item))));

    const { error } = await supabase.from('tasks').update({ status: nextStatus, due_date: nextDate }).eq('id', taskId);
