...
export function RecentActivity({ summary }: RecentActivityProps) {
  ...
  const tiles = [
    ...
    { label: 'Comentarios y recordatorios', value: counts.comments + counts.reminders, tone: 'border-emerald-100 bg-emerald-50 text-emerald-700' },
  ];
      ...
      <Card>
        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            <span className="inline-flex h-10 w-10 items-center justify-center rounded-[12px] bg-slate-100 text-slate-700">
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-2 2xl:grid-cols-4">
          {tiles.map((tile) => (
            <div key={tile.label} className={`min-h-[112px] rounded-[12px] border px-4 py-3 ${tile.tone}`}>
              <p className="line-clamp-2 text-[11px] font-semibold uppercase tracking-[0.16em]">{tile.label}</p>
              <p className="mt-3 text-3xl font-bold leading-none text-slate-950">{tile.value}</p>
            </div>
          ))}
        </div>
      </Card>
