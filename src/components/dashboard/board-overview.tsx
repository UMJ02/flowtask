        ...
        return (
          <Card key={item.label} className="rounded-[20px] border border-slate-200/85 p-5 transition hover:-translate-y-0.5 hover:">
            <div className="flex items-start justify-between gap-3">
              ...
              <span className={`inline-flex h-10 w-10 items-center justify-center rounded-2xl ring-1 ${item.accent}`}>
                <Icon className="h-5 w-5" />
              </span>
            </div>
            <div className="mt-5 flex items-end justify-between gap-3">
              <p className="text-[26px] font-semibold tracking-tight text-slate-900">{item.value}</p>
