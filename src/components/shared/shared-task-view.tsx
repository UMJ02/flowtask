      ...
      <main className="min-h-screen bg-slate-50 py-10">
        <div className="container-page max-w-5xl space-y-4">
          <Card>
            ...
            <h1 className="mt-2 text-xl font-bold text-slate-900">Vista compartida de tarea</h1>
    ...
    <main className="min-h-screen bg-slate-50 py-10">
      <div className="container-page max-w-5xl space-y-4">
        <Card className="border border-slate-100 bg-gradient-to-br from-white to-slate-50">
          <p className="text-sm font-semibold uppercase tracking-[0.2em] text-slate-500">FlowTask</p>
          <h1 className="mt-2 text-xl font-bold text-slate-900">Resumen ejecutivo de tarea</h1>
          <p className="mt-2 text-sm text-slate-600">Vista clara para revisión de estatus, seguimiento y fechas clave.</p>
        </Card>

        <div className="grid gap-4 md:grid-cols-3">
          <Card><p className="text-xs uppercase tracking-[0.2em] text-slate-500">Estado</p><p className="mt-2 text-xl font-bold text-slate-900">{data.task.status}</p></Card>
          <Card><p className="text-xs uppercase tracking-[0.2em] text-slate-500">Prioridad</p><p className="mt-2 text-xl font-bold text-slate-900">{data.task.priority}</p></Card>
          <Card><p className="text-xs uppercase tracking-[0.2em] text-slate-500">Deadline</p><p className="mt-2 text-xl font-bold text-slate-900">{data.task.due_date ? formatDate(data.task.due_date) : "—"}</p></Card>
        </div>
        ...
        <Card>
          <h3 className="text-base font-semibold text-slate-900">Seguimiento</h3>
