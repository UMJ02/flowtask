        ...
        <div className="pt-4">
          ...
          <div className="mt-4 divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white">
            {summary.departmentCapacity.length ? summary.departmentCapacity.slice(0, limit).map((item) => (
              <div key={item.name} className="flex items-start justify-between gap-3 px-4 py-4">
                <div>
        ...
        <div className="pt-4">
          ...
          <div className="mt-4 divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white">
            {summary.weeklyFocus.length ? summary.weeklyFocus.slice(0, limit).map((task) => (
              <Link key={task.id} href={asRoute(`/app/tasks/${task.id}`)} className="flex items-start justify-between gap-3 px-4 py-4 transition hover:bg-violet-50/40">
                <div>
        ...
        <div className="pt-4">
          ...
          <div className="mt-4 divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white">
            {summary.clientMomentum.length ? summary.clientMomentum.slice(0, limit).map((client) => (
              <Link key={client.id} href={asRoute(`/app/clients/${client.id}`)} className="flex items-start justify-between gap-3 px-4 py-4 transition hover:bg-violet-50/40">
                <div>
        ...
        <div className="pt-4">
          ...
          <div className="mt-4 divide-y divide-slate-200 rounded-2xl border border-slate-200 bg-white">
            {summary.projectPipeline.length ? summary.projectPipeline.slice(0, limit).map((project) => (
              <Link key={project.id} href={asRoute(`/app/projects/${project.id}`)} className="flex items-start justify-between gap-3 px-4 py-4 transition hover:bg-violet-50/40">
                <div>
...
export function PlanningCenter({ summary, compact = false }: { summary: PlanningOverview; compact?: boolean }) {
  ...
  return (
    <div className="space-y-4">
      <Card className="bg-[linear-gradient(135deg,#221b55_0%,#342d8d_58%,#151c4a_100%)] px-4 py-4 text-white shadow-[0_22px_54px_rgba(49,46,129,0.16)] md:px-5 md:py-5">
        <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
          <div className="max-w-3xl">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-violet-200">Planning center</p>
            <h2 className="mt-2 text-2xl font-bold md:text-3xl">Tu ventana de capacidad de los próximos 14 días</h2>
