...
export function ClientManagerPanel({ items, initialQuery = '' }: { items: ClientListItem[]; initialQuery?: string }) {
  ...
  return (
    <div className="space-y-3 pt-2 md:pt-3">
      <section className="grid items-start gap-4 xl:grid-cols-[minmax(0,1fr)_460px]">
        <div>
          <p className="text-xs font-extrabold uppercase tracking-[0.28em] text-emerald-600">Registros</p>
          <h1 className="mt-2 text-[20px] font-semibold tracking-[-0.028em] text-slate-950 md:text-[18px]">Catálogos del workspace para proyectos y tareas</h1>
          <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">Centraliza registros de clientes, departamentos y países para que después aparezcan en las listas desplegables de tareas y proyectos.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-3 xl:pt-1">
      ...
      <Card className="overflow-hidden rounded-[16px] border border-slate-200/90 bg-white">
        <div className="flex flex-wrap items-center gap-3 border-b ft-border px-3 py-4">
          {tabs.map((tab) => {
            const isActive = tab.key === activeTab;
            return (
              <button key={tab.key} type="button" onClick={() => setActiveTab(tab.key)} className={`inline-flex items-center gap-2 rounded-full px-3 py-3 text-sm font-bold transition ${isActive ? 'bg-slate-950 text-white' : 'border ft-border bg-white text-slate-700 hover:bg-slate-50'}`}>
        ...
        <div className="px-3 py-3">
          ...
          <div className="mb-5 flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
            <div className="relative w-full max-w-[440px]"><Search className="pointer-events-none absolute left-4 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={activeTab === 'clients' ? 'Buscar cliente...' : activeTab === 'departments' ? 'Buscar departamento...' : 'Buscar país...'} className="h-10 rounded-2xl ft-border bg-white pl-12 shadow-none" /></div>
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={() => { setQuery(''); setStatusFilter('all'); }} className="inline-flex h-10 items-center gap-2 rounded-2xl border ft-border bg-white px-3.5 text-[13px] font-bold text-slate-700 transition hover:bg-slate-50"><Filter className="h-4 w-4" /> Limpiar filtros</button>
              {activeTab === 'clients' ? (
                <label className="relative inline-flex h-10 items-center rounded-2xl border ft-border bg-white px-3.5 text-[13px] font-bold text-slate-700">
          ...
          <div className="mt-5 flex flex-col gap-3 text-sm text-slate-500 sm:flex-row sm:items-center sm:justify-between"><span>Mostrando {activeCount} de {currentTab.count} resultados</span><div className="flex items-center gap-2"><button type="button" className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border ft-border bg-white text-slate-400" disabled>‹</button><span className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-950 text-sm font-bold text-white">1</span><button type="button" className="inline-flex h-10 w-10 items-center justify-center rounded-2xl border ft-border bg-white text-slate-400" disabled>›</button><span className="ml-2 inline-flex h-10 items-center rounded-2xl border ft-border bg-white px-3.5 text-[13px] font-bold text-slate-600">10 / página</span></div></div>
        </div>
      </Card>

      {activeTab === 'clients' ? (
        <Card className="rounded-[16px] border border-slate-200/90 bg-white p-3">
