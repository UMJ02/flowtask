  ...
  const content = showHeader ? (
    <div>
      <button type="button" onClick={() => setOpen((value) => !value)} className="flex w-full items-center justify-between gap-3 rounded-2xl bg-slate-50 px-3 py-3 text-left transition hover:bg-slate-100">
        <div>
          <p className="text-sm font-semibold text-slate-900">Permisos del cliente</p>
          <p className="mt-1 text-sm text-slate-600">Revisa quién puede ver, editar o administrar clientes dentro de este espacio.</p>
        </div>
        <div className="flex items-center gap-2">
          {canManage ? <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-emerald-100">Puedes gestionarlos</span> : null}
