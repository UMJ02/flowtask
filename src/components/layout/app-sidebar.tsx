    ...
    <aside className="hidden min-w-0 bg-[linear-gradient(180deg,#071120_0%,#0A1730_100%)] text-[#E2E8F0] shadow-[16px_0_40px_rgba(2,6,23,0.08)] md:relative md:flex md:min-h-screen md:w-full md:self-stretch md:flex-col md:overflow-hidden">
      ...
      {collapsed ? (
        <div className="mb-2 flex shrink-0 justify-center">
          <button type="button" onClick={toggle} aria-label="Expandir menú" className="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-[14px] border border-white/10 bg-white/[0.04] text-slate-300 transition-colors hover:bg-white/[0.08] hover:text-white">
      ...
      <div className={`relative z-20 shrink-0 border-t border-white/10 bg-[#0A1730] pt-3 ${collapsed ? 'px-2 pb-4' : 'px-4 pb-5'}`}>
        ...
        {!collapsed ? (
          <button type="button" onClick={toggle} className="mt-4 flex h-11 w-full items-center justify-center gap-3 rounded-[14px] border border-white/10 bg-white/[0.035] text-sm font-semibold text-slate-300 transition-colors hover:bg-white/[0.07] hover:text-white">
            <PanelLeftClose className="h-4 w-4" />
            Colapsar menú
          </button>
        ) : null}
