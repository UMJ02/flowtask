...
export function OrganizationSwitcher({
  ...
  return (
    <div className="relative min-w-0 w-full max-w-[280px]">
      <button
        type="button"
        onClick={() => setOpen((value) => !value)}
        className="flex w-full items-center justify-between gap-3 rounded-[26px] border border-slate-200 bg-slate-50 px-4 py-3 text-left transition hover:border-emerald-200 hover:bg-white"
      >
        <div className="min-w-0 flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500">Rol: {formatRole(activeOrganization?.role)}</p>
          </div>
        </div>
      ...
      {open ? (
        <div className="absolute right-0 top-[calc(100%+10px)] z-30 w-full rounded-[24px] border border-slate-200 bg-white p-3 shadow-[0_24px_50px_rgba(15,23,42,0.16)]">
