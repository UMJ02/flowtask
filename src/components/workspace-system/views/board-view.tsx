...
export function BoardView({ tasks }: { tasks: WorkspaceTaskItem[] }) {
  ...
  const columns = [
    { label: "En curso", items: grouped.inProgress, color: "bg-emerald-500", hint: "Trabajo activo" },
    { label: "Pendiente", items: grouped.waiting, color: "bg-amber-500", hint: "Por desbloquear" },
    { label: "Completadas", items: grouped.completed, color: "bg-emerald-500", hint: "Cerradas" },
  ];
        ...
        <section key={col.label} className="rounded-[24px] border border-[var(--ft-workspace-border)] bg-white p-4 shadow-[var(--ft-workspace-shadow)]">
          <header className="mb-3 flex items-center justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-xs font-bold text-slate-400">{col.hint}</p>
            </div>
          ...
          <div className="space-y-3">
            {col.items.length ? col.items.map((task) => (
              <article key={task.id} className="ft-ws-row ft-ws-board-interactive-card rounded-[18px] border border-[var(--ft-workspace-border)] bg-slate-50 p-3 transition hover:bg-white hover:shadow-sm">
