...
function buildDefaultTitle(view: WorkspaceViewId, existing: WorkspaceProjectViewPreference[]) {
...
export function WorkspaceSavedViewsManager({
  ...
  const [title, setTitle] = useState(buildDefaultTitle(activeView, projectViews));
  ...
  async function saveCurrentView() {
    ...
    const { error } = await supabase.from("project_views").insert(payload).select("id").single();
    setBusy(null);

    if (error) {
      setFeedback({ tone: "error", text: error.message || "No se pudo guardar la vista." });
      return;
    }

    setFeedback({ tone: "success", text: "Vista guardada correctamente." });
    setTitle(buildDefaultTitle(activeView, [...projectViews, { id: "temp", projectId: context.projectId, viewType: activeView, title: cleanTitle, config: {}, isDefault: false, sortOrder: projectViews.length + 1 }]));
    router.refresh();
  }
  ...
  async function renameView(viewId: string) {
    ...
    if (!cleanTitle) {
      setFeedback({ tone: "error", text: "El nombre de la vista no puede quedar vacío." });
      return;
    }

    setBusy(`rename:${viewId}`);
    setFeedback(null);
    const { error } = await supabase.from("project_views").update({ title: cleanTitle }).eq("id", viewId).select("id").single();
    setBusy(null);

    if (error) {
      setFeedback({ tone: "error", text: error.message || "No se pudo renombrar la vista." });
      return;
    }
  ...
  async function setDefaultView(view: WorkspaceProjectViewPreference) {
    ...
    const { error } = await supabase.from("project_views").update({ is_default: true }).eq("id", view.id).select("id").single();
    setBusy(null);

    if (error) {
      setFeedback({ tone: "error", text: error.message || "No se pudo marcar como default." });
      return;
    }

    setFeedback({ tone: "success", text: "Vista marcada como predeterminada." });
    router.refresh();
  }
  ...
  async function deleteView(viewId: string) {
    ...
    setFeedback(null);
    const { error } = await supabase.from("project_views").delete().eq("id", viewId);
    setBusy(null);

    if (error) {
      setFeedback({ tone: "error", text: error.message || "No se pudo eliminar la vista." });
      return;
    }
    ...
    <section className="ft-ws-saved-views-manager ft-ws-enter">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          <p className="text-[11px] font-black uppercase tracking-[.18em] text-emerald-600">Vistas guardadas</p>
          <h3 className="mt-1 text-lg font-black tracking-[-.03em] text-slate-950">Saved Views Manager</h3>
          <p className="mt-1 max-w-3xl text-sm font-semibold text-slate-500">
            Guarda configuraciones de Lista, Board, Timeline, Tabla, Canvas, Archivos o Reportes dentro del proyecto activo sin reemplazar las rutas actuales.
          </p>
        </div>
        {onClose ? (
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(300px,420px)]">
        <div className="rounded-[22px] border border-slate-200/80 bg-white/82 p-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            />
            <button type="button" onClick={() => void saveCurrentView()} disabled={!canPersist || busy === "create"} className="ft-ws-active h-11 rounded-[16px] px-5 text-sm font-extrabold disabled:cursor-not-allowed disabled:opacity-55">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 2xl:grid-cols-3">
        {sortedViews.length ? sortedViews.map((view) => {
          const isBusy = busy?.endsWith(view.id);
          const isEditing = editingId === view.id;
          return (
            <article key={view.id} className="ft-ws-saved-view-card">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <span className="ft-ws-saved-view-type">{viewLabels[view.viewType] ?? view.viewType}</span>
                  {isEditing ? (
                    <input className="ft-ws-saved-view-title-input" value={editingTitle} onChange={(event) => setEditingTitle(event.target.value)} autoFocus />
                  ) : (
                    <h4 className="mt-2 truncate text-base font-black text-slate-950">{view.title}</h4>
                  )}
                  <p className="mt-1 text-xs font-bold text-slate-500">Orden {view.sortOrder} · {view.isDefault ? "Predeterminada" : "Guardada"}</p>
                </div>
                {view.isDefault ? <span className="ft-ws-saved-view-default"><Star className="h-3.5 w-3.5" /> Default</span> : null}
              </div>

              <div className="mt-4 flex flex-wrap gap-2">
                <button type="button" onClick={() => openView(view.viewType)} className="ft-ws-saved-view-action"><Eye className="h-3.5 w-3.5" /> Abrir</button>
                {isEditing ? (
                  <button type="button" onClick={() => void renameView(view.id)} disabled={isBusy} className="ft-ws-saved-view-action text-emerald-700"><CheckCircle2 className="h-3.5 w-3.5" /> Guardar</button>
                ) : (
                  <button type="button" onClick={() => { setEditingId(view.id); setEditingTitle(view.title); }} className="ft-ws-saved-view-action"><Pencil className="h-3.5 w-3.5" /> Renombrar</button>
                )}
                <button type="button" onClick={() => void setDefaultView(view)} disabled={isBusy || view.isDefault} className="ft-ws-saved-view-action"><Star className="h-3.5 w-3.5" /> Default</button>
                <button type="button" onClick={() => void deleteView(view.id)} disabled={isBusy} className="ft-ws-saved-view-action text-rose-700"><Trash2 className="h-3.5 w-3.5" /> Eliminar</button>
              </div>
            </article>
          );
        }) : (
          <div className="rounded-[22px] border border-dashed border-slate-300 bg-white/70 p-6 text-sm font-bold text-slate-500 md:col-span-2 2xl:col-span-3">
            No hay vistas guardadas para este proyecto todavía. Guarda la vista activa para empezar a personalizar el workspace.
          </div>
        )}
      </div>
    </section>
