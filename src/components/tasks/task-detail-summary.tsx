    ...
    <Card className="rounded-[34px] border border-slate-200/85 bg-white p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)] md:p-6">
      <div className="space-y-5">
        <div className="flex items-start justify-between gap-3">
          <Link href={taskListRoute(currentQuery)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 transition hover:bg-slate-50 hover:text-slate-900" aria-label="Volver al listado">
            <span aria-hidden>←</span>
          </Link>
          <div className="flex flex-wrap items-center justify-end gap-2">
            <EntityMemoryActions entity={{ id: task.id, type: 'task', title: task.title, subtitle: task.client_name || "Tarea", href: taskDetailRoute(task.id, currentQuery), updatedAt: task.updated_at ?? task.created_at ?? task.due_date ?? '1970-01-01T00:00:00.000Z' }} compact />
            ...
            <Link href={taskEditRoute(task.id, currentQuery)} className="inline-flex h-12 items-center justify-center rounded-[22px] border border-slate-200 bg-white px-6 text-sm font-semibold text-slate-900 transition hover:bg-slate-50">
              Editar tarea
            </Link>
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 md:text-4xl">{task.title}</h1>
