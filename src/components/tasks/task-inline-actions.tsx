...
import { buildTaskStatusUpdate } from '@/lib/tasks/status-rules';
...
export function TaskInlineActions({ taskId, status }: { taskId: string; status: string }) {
  ...
  const handleStatus = async (nextStatus: string) => {
    ...
    const { error } = await supabase.from('tasks').update(buildTaskStatusUpdate(nextStatus, null)).eq('id', taskId);
