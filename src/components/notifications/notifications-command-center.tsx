...
export function NotificationsCommandCenter({
  ...
  const cadenceLabel = preferences?.delivery_frequency === 'daily'
  ...
  return (
    <Card className="bg-[linear-gradient(135deg,#062b2a_0%,#0f172a_58%,#111827_100%)] text-white shadow-[0_24px_60px_rgba(2,6,23,0.2)]">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="max-w-3xl">
          ...
          <p className="mt-2 text-sm text-slate-300 md:text-base">
            Una cabecera más compacta para enfocarte en buscar, filtrar y resolver notificaciones sin ruido visual.
          </p>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:min-w-[360px]">
          ...
          <div className="rounded-[22px] bg-white/10 px-4 py-3 ring-1 ring-white/10">
            ...
            <p className="mt-1 text-xs text-slate-300">{deliverySummary.total ? `${deliverySummary.total} evento(s) de entrega registrados` : 'Aún no hay historial de entregas.'}</p>
          </div>
        </div>
      </div>
    </Card>
