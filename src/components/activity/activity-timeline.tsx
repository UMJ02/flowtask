      ...
      <div className="mt-4 space-y-3">
        {visibleItems.length ? (
          visibleItems.map((item) => {
            ...
            return (
              <div key={item.id} className={`rounded-2xl border border-slate-200 bg-white ${compact ? "px-3 py-2" : "px-4 py-3"}`}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className={`${compact ? "text-sm" : "text-sm"} font-medium text-slate-900`}>{labels[item.action] ?? item.action}</p>
                  </div>
                  <p className="text-xs text-slate-500">{formatDate(item.created_at)}</p>
                </div>
                {detail.title ? <p className={`${compact ? "mt-1 text-[13px]" : "mt-2 text-sm"} text-slate-700`}>{detail.title}</p> : null}
                {detail.role ? (
                  <p className={`${compact ? "mt-1" : "mt-2"} text-xs text-slate-500`}>
                ...
                {detail.status ? <p className={`${compact ? "mt-1" : "mt-2"} text-xs text-slate-500`}>Estado: {detail.status}</p> : null}
              </div>
            );
          })
        ) : (
