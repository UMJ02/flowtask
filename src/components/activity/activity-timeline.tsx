...
export function ActivityTimeline({
  ...
  return (
    <Card className={compact ? "border border-slate-200/90 bg-white/[0.96]" : undefined}>
      <div>
              ...
              <div key={item.id} className={compact ? "rounded-xl border border-slate-200 bg-white px-3 py-2.5" : "rounded-2xl border border-slate-200 bg-white px-4 py-3"}>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <p className={compact ? "text-[13px] font-medium text-slate-900" : "text-sm font-medium text-slate-900"}>{labels[item.action] ?? item.action}</p>
                  </div>
