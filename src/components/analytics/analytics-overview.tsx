...
export function AnalyticsOverview({ summary, compact = false }: { summary: WorkspaceAnalyticsSummary; compact?: boolean }) {
  ...
  return (
    <div className="space-y-3.5">
      <Card className="border-cyan-200/70 bg-[linear-gradient(135deg,rgba(5,46,43,0.98)_0%,rgba(11,57,84,0.97)_44%,rgba(15,23,42,0.98)_100%)] px-4 py-3 text-white shadow-[0_14px_30px_rgba(8,47,73,0.14)] md:px-5 md:py-3.5">
        <div className="flex flex-col gap-3.5 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl min-w-0">
            ...
            <p className="mt-1 max-w-2xl text-[13px] leading-6 text-slate-100/88 sm:text-[14px]">
          ...
          <div className="grid gap-2 sm:grid-cols-2 xl:grid-cols-3 lg:min-w-[420px] xl:min-w-[560px]">
            {heroCards.map(({ label, value, helper, tone }) => (
...
function HeroMetric({ label, value, helper, tone }: { label: string; value: string; helper: string; tone: 'sky' | 'violet' | 'amber' | 'emerald' | 'rose' }) {
  ...
  return (
    <div className={`rounded-[14px] border px-3 py-2.5 backdrop-blur-sm ${theme}`}>
      <p className="text-[11px] uppercase tracking-[0.16em] text-cyan-100/80">{label}</p>
      <p className="mt-2 text-[1.7rem] font-bold leading-none text-white">{value}</p>
    ...
    <Card>
      ...
      <div className="mt-4 space-y-2.5">
        {visibleItems.length ? visibleItems.map((item) => (
          <div key={`${item.source}-${item.id}`} className="rounded-[16px] border border-slate-200 bg-white px-3 py-3 transition hover:-translate-y-0.5 hover:border-cyan-200 hover:shadow-[0_12px_26px_rgba(15,23,42,0.08)]">
