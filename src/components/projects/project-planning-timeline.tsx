...
export function ProjectPlanningTimeline({ project, tasks, currentQuery = "" }: ProjectPlanningTimelineProps) {
  ...
  return (
    <section id="timeline" className="ft-project-timeline-panel scroll-mt-28 p-3">
      ...
      <div className="mt-5 flex justify-center xl:justify-end">
        <button type="button" aria-pressed={showBuilder} onClick={() => setShowBuilder((v) => !v)} className={cn("inline-flex h-10 items-center gap-2 rounded-[14px] px-3 text-sm font-semibold ring-1 transition", showBuilder ? "bg-[#050B18] text-white ring-[#050B18]" : "bg-[#ECFDF5] text-[#087A4B] ring-[#BBF7D0]")}><Settings2 className="h-4 w-4" />Ajustes de vista</button>
      </div>

      <div className={cn("mt-6 grid gap-3", showBuilder ? "xl:grid-cols-[minmax(0,1fr)_320px]" : "xl:grid-cols-1")}>
        <div className="min-w-0">
          <div className="ft-subcard mb-4 flex flex-col gap-3 bg-[#FBFCFE] p-3 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-2">
              <button type="button" onClick={() => setOffset((v) => v - 1)} className="inline-flex h-9 w-9 items-center justify-center rounded-[12px] border ft-border bg-white">‹</button>
          ...
          {visibleTasks.length ? (
            <div className="overflow-x-auto rounded-[16px] border ft-border">
              <div className="min-w-[980px] grid grid-cols-[320px_1fr] bg-white">
                <div className="border-r ft-border">
                  <div className="h-10 border-b ft-border bg-slate-50/80 px-4 py-4 text-[11px] font-semibold uppercase tracking-[0.14em] ft-text-muted">Tarea / Responsable</div>
                  {groupedTasks.map((group) => (
                    <div key={group.label}>
                      <div className="border-b ft-border bg-[#F8FAFC] px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.14em] ft-text-muted">{group.label}</div>
                ...
                <div className="relative">
                  <div className="grid h-10 border-b ft-border bg-slate-50/80" style={{ gridTemplateColumns: `repeat(${days.length}, minmax(40px, 1fr))` }}>
                  ...
                  {groupedTasks.map((group) => (
                    <div key={group.label}>
                      <div className="h-[33px] border-b ft-border bg-[#F8FAFC]" />
                      {group.tasks.map((task) => {
            ...
            <div className="rounded-[16px] border border-dashed border-[#CBD5E1] bg-[#F8FAFC] px-3 py-4 text-center">
        ...
        {showBuilder ? (
          <aside className="rounded-[16px] border ft-border bg-white p-3">
            ...
            <div className="mt-5 space-y-4">
              <label className="flex items-center justify-between rounded-[14px] border ft-border bg-[#F8FAFC] px-4 py-3 text-sm font-bold ft-text-main">Incluir concluidas<input type="checkbox" checked={showConcluded} onChange={(e) => setShowConcluded(e.target.checked)} /></label>
              <label className="block text-xs font-semibold uppercase tracking-[0.14em] ft-text-muted">Colores<select value={colorBy} onChange={(e) => setColorBy(e.target.value as ColorMode)} className="mt-2 h-11 w-full rounded-[14px] border ft-border bg-white px-3 text-sm font-bold ft-text-main"><option value="status">Por estado</option><option value="priority">Por prioridad</option><option value="responsible">Por responsable</option></select></label>
              <label className="block text-xs font-semibold uppercase tracking-[0.14em] ft-text-muted">Agrupar por<select value={groupBy} onChange={(e) => setGroupBy(e.target.value as GroupMode)} className="mt-2 h-11 w-full rounded-[14px] border ft-border bg-white px-3 text-sm font-bold ft-text-main"><option value="none">Sin agrupar</option><option value="status">Estado</option><option value="priority">Prioridad</option></select></label>
