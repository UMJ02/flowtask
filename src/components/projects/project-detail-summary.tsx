...
export function ProjectDetailSummary({ project, currentQuery = "" }: { project: any; currentQuery?: string }) {
  ...
  return (
    <Card className="rounded-[34px] border border-slate-200/85 bg-[linear-gradient(180deg,#ffffff_0%,#f8fbff_100%)] p-5 shadow-[0_18px_40px_rgba(15,23,42,0.06)] md:p-6">
      <div className="flex flex-col gap-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="min-w-0 flex-1">
            <Link href={projectListRoute(currentQuery)} className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-500 transition hover:text-slate-900" aria-label="Volver al listado">
          ...
          <div className="flex flex-wrap items-center gap-2 lg:max-w-[620px] lg:justify-end">
            ...
            <Link href={projectEditRoute(project.id, currentQuery)} className="inline-flex h-12 items-center justify-center rounded-[20px] border border-slate-200 bg-white px-6 text-sm font-semibold text-slate-900 transition hover:bg-slate-50">
              Editar proyecto
            </Link>
            <button
              type="button"
              onClick={() => setDetailsOpen((value) => !value)}
              className="inline-flex h-12 items-center gap-2 rounded-[20px] border border-slate-200 bg-white px-5 text-sm font-semibold text-slate-700 transition hover:bg-slate-50"
            >
