    ...
    <section className="relative overflow-hidden rounded-[24px] border border-[#BBF7D0] bg-white p-5">
      ...
      <div className="relative grid gap-5 lg:grid-cols-[230px_minmax(0,1fr)_360px]">
        ...
        <div className="space-y-4">
          ...
          <Input value={form.title} onChange={(event) => setField("title", event.target.value)} className="min-h-[58px] rounded-[18px] border-[#E5EAF1] bg-white px-4 text-[28px] font-semibold tracking-[-0.035em] text-[#0F172A] focus:border-[#16C784] focus:ring-4 focus:ring-emerald-500/10" placeholder="Nombre del proyecto" />
        ...
        <div className="flex flex-col justify-between gap-5">
          ...
          <div className="grid gap-3">
            <label className="space-y-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500"><span className="flex items-center gap-2"><CheckCircle2 className="h-4 w-4" />Estado</span><Select value={form.status} onChange={(event) => setField("status", event.target.value)} className="h-11 rounded-2xl border-[#E5EAF1] bg-white font-semibold">{PROJECT_STATUSES.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</Select></label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <label className="space-y-2 text-xs font-semibold uppercase tracking-[0.16em] text-slate-500"><span className="flex items-center gap-2"><Globe2 className="h-4 w-4" />País</span><Select value={form.country} onChange={(event) => setField("country", event.target.value)} className="h-11 rounded-2xl border-[#E5EAF1] bg-white font-semibold"><option value="">País</option>{countryOptions.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</Select></label>
