...
const required = [
  ...
  'docs/release/V58_11_5_ASSISTANT_PREMIUM_CARD_REFINE.md',
];

const missing = required.filter((item) => !fs.existsSync(path.join(root, item)));
if (missing.length) {
  console.error('[verify-v58.11.5] FAILED');
  for (const file of missing) console.error(` - Missing ${file}`);
  process.exit(1);
}

const version = fs.readFileSync(path.join(root, 'src/lib/release/version.ts'), 'utf8');
if (!version.includes('58.11.5-assistant-premium-card-refine')) {
  console.error('[verify-v58.11.5] FAILED - release version mismatch');
  process.exit(1);
}

console.log('[verify-v58.11.5] OK — card premium del asistente refinado y validado sobre la base v58.11.4.');
