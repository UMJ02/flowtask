...
if (missing.length) {
  console.error("[verify:v58.15.3.8] Missing files:", missing.join(", "));
  process.exit(1);
}
...
const checks = [
  ["release version aligned", release.includes("58.15.3.8-task-editor-full-bleed-rounded-hero")],
  ["task editor uses full-bleed app-shell correction", taskForm.includes("-mx-4 min-h-screen") && taskForm.includes("md:-mx-6") && taskForm.includes("rounded-[28px]") && taskForm.includes("grid w-full gap-4 px-3") && !taskForm.includes("max-w-[1840px]")],
...
if (failed.length) {
  console.error("[verify:v58.15.3.8] Failed checks:");
  for (const [label] of failed) console.error(`- ${label}`);
  process.exit(1);
}
console.log("[verify:v58.15.3.8] OK — Task editor full-bleed gutters + rounded hero + project/client media aligned.");
