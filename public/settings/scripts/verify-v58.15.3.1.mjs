...
if (missing.length) {
  console.error("[verify:v58.15.3.1] Missing files:", missing.join(", "));
  process.exit(1);
}
...
const checks = [
  ["release version aligned", release.includes("58.15.3.1-task-detail-premium-2026")],
...
if (failed.length) {
  console.error("[verify:v58.15.3.1] Failed checks:");
  for (const [label] of failed) console.error(`- ${label}`);
  if (forbiddenFound.length) console.error(`- Forbidden fields found: ${forbiddenFound.join(", ")}`);
  process.exit(1);
}

console.log("[verify:v58.15.3.1] OK — Task Detail Premium 2026 aligned with DB-safe fields.");
